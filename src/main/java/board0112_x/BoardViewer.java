...
public class BoardViewer {
    ...
    public void printOne(int id) {
        ...
        commentViewer.printList(id);

        String message;
        int userChoice;

        if(boardDTO.getWriterId() == logIn.getId()) {
            message = "1. 수정 2. 삭제 3. 댓글 등록 4. 댓글 수정 5. 댓글 삭제 6. 뒤로 가기";
            userChoice = ScannerUtil.nextInt(SCANNER, message, 1, 6);
        } else {
            message = "3. 댓글 등록 4. 댓글 수정 5. 댓글 삭제 6. 뒤로 가기";
            userChoice = ScannerUtil.nextInt(SCANNER, message, 3, 6);
        }
        ...
        if (userChoice == 1) {
            ...
            commentViewer.writeComment(id);
        } else if (userChoice == 4) {
            commentViewer.updateComment(logIn.getId());
        } else if (userChoice == 5) {
            commentViewer.deleteComment(logIn.getId());
